...
def calcTime():
    ...
    repetitions = 1000
    model = Model(mode='b0', n_class=12, inputs='rgbt', fusion_mode='cat').to(device)
    # from ptflops import get_model_complexity_info
    # flops, params = get_model_complexity_info(model, (3, 480, 640), as_strings=True, print_per_layer_stat=False)
    # print('Flops ' + flops)
    # print('Params ' + params)
    dummy_input1 = torch.rand(1, 3, 512, 640).to(device)
    dummy_input2 = torch.rand(1, 3, 512, 640).to(device)
